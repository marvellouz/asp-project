...
var form = "<h1>Моите имоти:</h1>"
place = connection.execute("select * from Customer_has_Place cp join place_info p on cp.Place_Seller_User_email = p.Seller_User_email where cp.Customer_User_email = '" + user["email"] + "';");
while(!place.eof) {
  form += "<hr/><strong>Тип:</strong> " + place("Type_name").value;
  form += "<br/><strong>Квадратура:</strong> " + place("meters").value + " кв. м." + "<br/><strong>Цена:</strong> " + place("price").value + " лв.";
